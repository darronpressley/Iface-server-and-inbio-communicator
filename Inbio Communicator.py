...
def inbio_communicate():
    ...
    if ret == -1: return -1
    #test if spooling transactions
    check_spool_transactions()
...
def poll_inbio(ip_address, terminal_id):
    ...
    if ret < 0:
        commpro.Disconnect(hcommpro)
        ret_error(ip_address, ret,function+","+str(table.raw))
        #we are not emailing this was just a test but keep for furture reference of the code
        #emailer.email_txt(function+","+str(table.raw),"darronpressley@gmail.com")
        return 0
    else:
        ...
        if test_insert_transactions:
            ...
            if ret < 0:
                ret_error(ip_address, ret, function+","+str(p_table.raw))
                commpro.Disconnect(hcommpro)
                return 0
        #set time
        ...
        if ret < 0:
            ret_error(ip_address, ret, function+","+str(p_items.raw))
            commpro.Disconnect(hcommpro)
            return 0
    commpro.Disconnect(hcommpro)
    return 0
...
def check_spool_transactions():
    ...
    ret = sqlconns.sql_select_single_field("SELECT TOP 1 value FROM d_inbio_misc WHERE property like 'SPOOL_TRANSACTIONS'")
    if ret=="1":
            ...
            if len(employee_list) > 0:
                ...
                for zz in range(len(term_list)):
                    ...
                    for y in range(len(datalist)):
                        ...
                        term_policy_string += str(term_list[zz][0]) + """,True"""
                        if term_policy_string in datalist[y]:
                            ...
                            if int(auth_timezone_id) > 0:
                                ...
                                if ret < 0:
                                    ...
                                    if ret == -1: return -1
                                    return 0
                                employee_fingers = sqlconns.sql_select_into_list("SELECT size,fid,tmp from d_iface_finger WHERE employee_id = " + str(employee_list[0][0]))
                                if employee_fingers ==-1:
                                    ...
                                    return 0
                                p_fingers = create_string_buffer(str.encode("templatev10"))
                                emp_pin = create_string_buffer(str.encode('Pin='+str(employee_list[0][0])))
                                for z in range(len(employee_fingers)):
                                    ...
                                    p_data = create_string_buffer(str.encode(p_data))
                                    ret = commpro.SetDeviceData(hcommpro, p_fingers, p_data, "")
                                    if ret < 0:
                                        ret_error(ip_address, ret, function)
                                        commpro.Disconnect(hcommpro)
                                        ret = completed_failure(str(event_list[index][1]))
...
def insert_transactions(poll_list, terminal_id):
    ...
    for index in range(length):
        if index != 0:
            ...
            if SPOOL_TRANSACTIONS == 1:
                spool_log(cardno, pin, verified, door_id, event, date_and_time,terminal_id)
            if event != "0":
                ret = insert_sql_transaction(pin, date_and_time, event, door_id, terminal_id,inoutstate)
                if ret == -1:
                    return -1
                if ret < 0: bool = False
...
def insert_sql_transaction(empid, dte, event, door_id, terminal_id,inoutstate):
    function = inspect.stack()[0][3]
    try:
        if inoutstate == "1" and event == 1: event = 2
        if inoutstate == "1" and event == 3: event = 4
    except Exception as e:
        ret_error(str(terminal_id), -1,function+","+str(inoutstate) + " " +str(e))
...
def spool_log(cardno, pin, verified, door_id, event, date_and_time,terminal_id):
    try:
        e = open(gl.SPOOL_LOG, 'a')
        e.write(cardno + "," + pin + "," + verified + "," + door_id + "," + event + "," + date_and_time + "," + str(terminal_id) + """\n""")
        e.close
    except:
        return
