...
APPNAME = "Iface PUSH"
ACCESS_TERMINAL = 4
ATTENDANCE_TERMINAL = 10
ERROR_LOG =  "error_log"
SERVER_STARTED = 0

class AppServerSvc(win32serviceutil.ServiceFramework):
    _svc_name_ = "iface server"
    _svc_display_name_ = "iface server"
              ...
              #      county = 0
               #     dte = str(datetime.now())
                #    e = open("c:/temp/1log.txt", 'a')
                 #   e.write(dte + " " + str(SERVER_STARTED) + """\n""")
                  #  e.close()
                #if SERVER_STARTED ==0:
                 #   SERVER_STARTED =1
                  #  tester()
                    #server_class = http.server.HTTPServer
                    #httpd = server_class((HOST_NAME, gl.server_port), MyHandler)
                    #try:
                    #    log_initialise()
                    #    httpd.serve_forever()
    ...
    def self_log_initialise(self):
        #f.error_logging(APPNAME, "Service started.", "error_log","")
        dte = str(datetime.now())
        e = open("c:/temp/3log.txt", 'a')
        e.write(dte + """looginit_test\n""")
        e.close()


def tester():
    dte = str(datetime.now())
    e = open("c:/temp/2log.txt", 'a')
    e.write(dte + """tester\n""")
    e.close()
    test()

def main_function():
    f.error_logging(APPNAME, "tested.", "error_log","")
...
def error_initialise():
    f.error_logging(APPNAME, "Error starting Server.", "error_log","")
...
class test():
    def __init__(self):
        ...
        e.close()
        server_class = http.server.HTTPServer
        httpd = server_class((HOST_NAME, 82), MyHandler)
        try:
            httpd.serve_forever()
        except Exception as e:
            error_initialise()
